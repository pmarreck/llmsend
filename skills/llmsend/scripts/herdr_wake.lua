...
local function lines(screen)
	...
	for raw in (screen:gsub('\r\n','\n')..'\n'):gmatch('(.-)\n') do
		...
		while i<=#raw do
			...
			while n<=#codes do
				local c=codes[n]
				if c==0 then dim=false
				elseif c==2 then dim=true
				elseif c==22 then dim=false
		...
		result[#result+1]={text=text,solid=solid,background=background}
	end
	return result
end
...
function M.composer(agent, screen)
	...
	if agent=='codex' and not rows[start].background then return unknown end
	...
	for i=start,#rows do
		local r=rows[i]
		if i>start then
			if agent=='codex' and not r.background then
...
function M.run(port, options)
	...
	return finish(submitted and 'unconfirmed' or 'deferred','observation-deadline')
end
return M
